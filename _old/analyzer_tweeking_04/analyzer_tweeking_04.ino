...
void updateSpectrum() {
  static double dynamicMax = 500.0;
  ...
  for (int band = 0; band < MATRIX_WIDTH; band++) {
    ...
    static double noiseFloor[MATRIX_WIDTH] = {0};
    noiseFloor[band] = 0.95 * noiseFloor[band] + 0.05 * magnitude;
    magnitude = magnitude - noiseFloor[band];
    if (band == 1 && magnitude < 50) magnitude = 0;  // Stronger gate for lowest band
    if (magnitude < 5) magnitude = 0;
    if (magnitude > dynamicMax) dynamicMax = magnitude;
    dynamicMax = 0.95 * dynamicMax + 0.05 * magnitude;
    int height = map(magnitude, 0, dynamicMax, 0, MATRIX_HEIGHT);
    height = constrain(height, 0, MATRIX_HEIGHT);
    if (height > 1) drawColumn(band, height);
  }
  strip.show();
}
