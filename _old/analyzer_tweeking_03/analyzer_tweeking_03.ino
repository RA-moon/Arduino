...
void setup() {
  ...
  delay(2000);
  strip.begin();
  strip.setBrightness(64);  // Set to 25% brightness
  strip.show();

  // Debug output: print LED values per column
  Serial.print("LED Heights: ");
  for (int band = 0; band < MATRIX_WIDTH; band++) {
    int index = band + 1;
    double magnitude = vReal[index];
    double gain = 500.0 / (dynamicMax[band] + 1);
    gain = constrain(gain, 0.5, 8.0);
    int height = magnitude * gain;
...
void updateSpectrum() {
  static double dynamicMax[MATRIX_WIDTH] = {500.0};  // moving upper bound
  ...
  // BPM estimation based on energy
  long energy = 0;
  for (int i = 0; i < SAMPLE_SIZE; i++) {
    energy += abs(buffer[i] >> 14);
  }
  ...
  // Calculate dynamic median from all magnitudes
  double sumMagnitude = 0;
  for (int i = 1; i <= MATRIX_WIDTH; i++) {
    sumMagnitude += vReal[i];
  }
  double median = sumMagnitude / MATRIX_WIDTH;
  for (int band = 0; band < MATRIX_WIDTH; band++) {
    int index = band + 1;  // skip DC bin
    double magnitude = vReal[index];
    if (magnitude > dynamicMax[band]) dynamicMax[band] = magnitude;
    dynamicMax[band] = 0.95 * dynamicMax[band] + 0.05 * magnitude;
    double gain = 500.0 / (dynamicMax[band] + 1);
    gain = constrain(gain, 0.5, 8.0);
    int height = magnitude * gain;
    if (height < 1) height = 0;
