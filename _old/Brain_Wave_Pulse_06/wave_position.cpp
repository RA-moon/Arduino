...
void updateWaves() {
  ...
  waves.erase(
    std::remove_if(waves.begin(), waves.end(), [](const Wave& wave) {
      float outRight = 10.0f + wave.noseWidth + wave.tailWidth + 1.0f;
      float outLeft = -wave.tailWidth - wave.noseWidth - 1.0f;

      return (!wave.reverse && wave.center > outRight) ||
            (wave.reverse && wave.center < outLeft);
    }),
    waves.end()
  );
